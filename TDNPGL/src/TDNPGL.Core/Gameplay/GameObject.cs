...
namespace TDNPGL.Core.Gameplay
{
    public class GameObject : IParentable, IEquatable<GameObject>, IComparable, IUpdateable
    {
        ...
        {
            Scripts.ForEach((GameObjectScript script) =>
        ...
        public virtual void OnMouseDown(SKPoint point)
        {
            Scripts.ForEach((GameObjectScript script) =>
                ThreadPool.QueueUserWorkItem((object state) =>
                 script.OnMouseDown(point)));
        }
        public virtual void OnMouseDownOver(SKPoint point)
        {
            Scripts.ForEach((GameObjectScript script) =>
                ThreadPool.QueueUserWorkItem((object state) =>
                 script.OnMouseDownOver(point)));
        }
        internal void CollidedWith(GameObject obj)
        {
            Scripts.ForEach((GameObjectScript script) =>
                ThreadPool.QueueUserWorkItem((object state) =>
                 script.OnCollideWith(obj)));
        }
